# vagrant-umgebung-LA
Die Vagrant-Umgebung für die Lehrlinge der Bedag Informatik AG. Mit hilfe dieser Vagrant-Umgebung ist es einfach möglich, dass du dein eigenes, kleines, virtuelles Rechenzentrum in wenigen Schritten in Betrieb nehmen kannst. Alles was du dazu benötigst, erfährst du in der folgenden Anleitung.

## 1. Voraussetzungen
Um die Vagrant auf deinem Testgerät verwenden zu können, benötigst du folgende Dinge: 
...
4. Genügen freier Festplattenspeicher. Die Umgebung benötigt um die 40 bis 50 GB.

## 2. Erstmalige installation
Wenn du alle Voraussetzungen aus Schritt 1 erfüllst, so kannst du mit der Installation der Umgebung beginnen. Dazu klonst du dieses Repository auf dein Testgerät oder lädst das Zip herunter und entpackst es z.B. auf deinen Desktop oder einen anderen Ort, an dem du es gut finden kannst. Befolge nun folgende Schritte:
...
3. Starte nur mittels `vagrant up` die Umgebung. Dies kann je nach Internetgeschwindigkeit und Leistung deines Gerätes bis zu 90 Minuten dauern. Du kannst aber gut etwas anderes während dieser Zeit machen. 
...
In diesem Abschnitt findest du allerlei nützliche Tipps und Tricks rund um das Thema Vagrant im allgemeinen aber auch um diese spezifische Umgebung. 
...
3. `cli01`: Dies ist der Client, welcher auf Windows 10 2004 basiert. Auf dem Client sind ein paar Programme wie Firefox oder Notepad++ vorinstalliert. Auch hier kannst du dich via `vagrant rdp cli01` oder `vagrant powershell cli01` verbinden. 
